  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <Container component={Paper} sx={{ padding: 2, marginTop: 2, marginBottom: 2, borderRadius: 2, boxShadow: 3 }}>
        <Typography variant="h4" component="h1" gutterBottom>{post?.titulo}</Typography>
        <Typography variant="h4" component="h1" gutterBottom>{post?.conteudo}</Typography>
        <Typography variant="h4" component="h1" gutterBottom>{post?.autor}</Typography>
      </Container>
    </Box>
  );
};
